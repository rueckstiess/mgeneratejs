 ...
 * @param  {object} globalState         A global state for operators to persist
 *                                      data across mgenerate invocations
 * @return {Any}                        return the value after evaluation
 */
var evalTemplate = function(template, isValue, globalState) {
  return isValue
    ? evalValue(template, globalState)
    : evalObject(template, globalState);
};
 ...
 */
var callOperator = function(op, opts, globalState) {
  opts = opts || {};
  ...
  if (_.includes(opNames, op)) {
    return operators[op](
      (op, opts) => evalTemplate(op, opts, globalState),
      opts,
      globalState
    );
  }
 ...
 */
evalObject = function(template, globalState = {}) {
  var result = _.mapValues(template, v => evalValue(v, globalState));
 ...
 * @return {any}                return value after template is evaluated
 */
evalValue = function(template, globalState) {
  if (_.isString(template)) {
    if (_.startsWith(template, '$')) {
      return callOperator(template.slice(1), {}, globalState);
    }
  ...
  if (_.isPlainObject(template)) {
    ...
    if (_.startsWith(op, '$')) {
      ...
      return callOperator(op, options, globalState);
    }
    return evalObject(template, globalState);
  }
